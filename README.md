...
- [Data transmission and I/O models（数据的传输和io模型）](#数据的传输和io模型)
---
...
### 总结
在这一节中，我们一步一步从上到下的对示例代码的原理进行深挖，并由此理解了go网络传输的模型搭建和高性能的原因。以及和我们之前[c](#1socket-underlying-c)代码的逻辑一致的部分，从c语言走到了现代web网络搭建。我们通篇在讲网络，讲go的通信实现，不过在开始开始对这部分进行梳理之前先切换视角。
#### 现代语言和go语言选择的方向
* 在我糟糕的大学课本上有一句话叫`c生万物`，这句话基于这样一个幻想，整个计算机世界的底层是固定不变的，c，汇编等语言作为基石，上层语言百花齐放，各有千秋，但是他们还是要回归最底层。但是在这个自上而下对go语言机理的部分研究和youtube[Core Dumped 的科普视频](https://www.youtube.com/watch?v=7ge7u5VUSbE [[00:46](http://www.youtube.com/watch?v=7ge7u5VUSbE&t=46)])知道，这些所谓的底层语言的固定不过只是因为计算机中历史惯性的影响，就是ipv6尽管已经推行多年但先行的大多数网站依旧采用ipv4，[C 语言不是一种低级语言](Https://queue.acm.org/detail.cfm?id=3212479)这篇文章给我们提供了C语言和现代处理器的矛盾之处以及处理器为了适应c语言而做出的一系列硬件让步。对于现代cpu来说，多线程，矢量运算和多级缓存已经成为基本的设计，而c语言依旧采用扁平的内存设计，单线程的执行和编程思想，不仅不适应与现代硬件，而且还产生了很多问题
* 不同的语言都在尝试去解决这一矛盾，rust通过所有权来解决内存管理，通过检查器和内存周期来解决悬空指针，直接取消null。这些举措本质上是为了更严格的去指定语言规范来解决安全问题，尽管其在性能提升方面,如通过设计不支持链表使用，也有所建树，但是其出发点和设计思路就决定这门语言是对c的缝补而非从思想上的替换。而go则走向了另一个方向，go通过runtime管理使得内存维护不需用户处理，通过协程来实现高并发和高性能，但是runtime机制其微秒级别的时间损失让go无法处理底层需求，而自动化的内存管理在带来便利的同时同样也使得精准的变量控制无法实现。zig语言则通过显式分配器来管理内存，comptime来管理洪，染色函数来处理协程，试图从根本上来取代c。
* 但是我们仍然不可否认，c语言仍旧是如今占据市场教育主流的语言，也是我们理解计算机世界最好的选择之一，比起根本无法实现的完全放弃，和对这门语言的过度迷信，通过c来了解计算机的面貌并在这个过程中去理解现在的计算机发展和困境则是更好的选择。
#### go的网络体系
* 我们这里的讨论先从在c实现的[epoll](./01_webcoding_based_on_c/05_tcp/06_server_epoll.c)理解开始，go的网络体系大概可以分成runtime层和internal层，前者和垃圾处理器绑定并处理系统调用和并发，后者则是上层封装给用户的接口。listen函数首先创建显式传入连接方式的listen接口，这个接口包含对应的方法实现，这个创建在底层创建了netpoll轮询器和对应socket并将socket写入netpoll当中，并在runtime层去执行诸如bind的系统调用，accept函数则可以看作是对连接的独特read，在这里我们看到之前所提到的所有函数的一个交界点：[init](./02_go_sdk/go/src/internal/poll/fd_poll_runtime.go#L38)，对于listen而言，这样一个函数就是业务逻辑的终点，对于accept，read和write函数而言，这样一个函数只是上层封装函数的一个调用，go将初始化和socket写入封装到一起保证了netpoll在未初始化之前绝对不会有错误写入，在net包这里，pollfd和fd的严格功能区分不是我们关注的重点，我们只需要明白后者是实际的通用业务调用对接runtime层，前者则是上层独立协议封装和错误检查
* go关键字直接创建协程并将对应函数压入栈，这时候我们让新携程去处理对应网络连接而旧协程去不断阻塞调用accept来探测网络连接的到来。随后进入runtime层，这时候业务结构体变为polldesc，两者间的结构体通过储存下级结构体的版本号和地址来偶联。到这里，accept，read和write的函数来到了同一条路，[poll_runtime_pollWait](./02_go_sdk/go/src/runtime/netpoll.go#L336)进行错误检查和循环确保并进行rg，wg的字段选择
[netpollbloack](./02_go_sdk/go/src/runtime/netpoll.go#L548)进行原子交换对应polldesc的状态，[gopark](./02_go_sdk/go/src/runtime/proc.go#L443)则是切换g0栈进行解绑和协程挂起。在这里accept，read和write的区别就是在[poll_runtime_pollWait](./02_go_sdk/go/src/runtime/netpoll.go#L336)的字段选择不一致

在这里我们看到了go设计的核心，在底层设计通用调用接口，在上层针对对应功能实现独立封装，不同层级的结构体实现不同层级的的业务功能，彼此间又直接或简介的偶联在一起，将网络连接直接内化为底层设计的核心，并将其与gc绑定在一起来作为netpoll的动力，由此实现了go在web开发方面的优势