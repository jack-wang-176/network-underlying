...
* IP 是当今世界互联网的基石，承载了今天互联网中的主要数据传递，ip地址则让数据包从局域网的层面上升到全球互联网层面。将ip地址简单的理解为mac地址在网络上的映射略有问题，mac地址是表示计算机的唯一表示（准确的说应该是网卡），但是ip地址本身只是一个点位，一个被路由器临时分配在局域网中的点位，路由器决定了对应的ip段，在这个局域网中数据包向外通信先经过路由器，再发送给另一局域网中的主机。不知于单纯的数据包向外发送，我们极力避免的回环数据包的发送和传递也是基于特定的ip段。//todo总结一下ip段的功能
* 数据链路层的ethnet是所有计算机网络通信间必有的结构，而在网络层开始，根据不同的应用场景存在不同的协议头分化。
* 我们在这里介绍的这5中网络层协议族都是为了实现这么一个计算机网络世界的基本框架骨干。
  1. ip:底层网络基础，是最主要的通信方式.
  2. icmp:负责ip协议的诊断和报错，直接封装在ip协议包中，不需上层协议，通过我们后面所说的协议号进行标识，如我们常用的ping命令就是基于这个协议
  3. arp：严格来说这个协议介于网络层和数据链路层的中间，为了满足ip和mac地址间的映射翻译，将ip地址和mac地址对应在一起
  4. igmp： 是一种一对多的高效数据包发送，常用于直播中
  5. ipsec：对ip包进行加密验证，常用于企业级的vpn搭建
  //todo：增加这几种网络协议的描述
* 为了保证逻辑的连贯性，我们在这部分中只去介绍arp和ip协议，前者将ip地址和mac地址对应起来，后者则真正打开了网络通信的大门。由于包头变得更加复杂，为了便于观看，之后我们表示数据包报头都采用规范格式。并且只去关注几个最核心的字段。
...
**1**. **ARP**
    ...
    +-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+-+
    ```
 * hardware: 通常是一，代表ethnet头
 * protocol type：作为mac地址和ip地址的翻译官服务的协议类型，例如0x8000代表ipv4标识
 * HW / Proto Size：分别是mac地址和ip地址
 * Opcode：代表不同的arp类型，值1代表arp广播，值2代表对应ip地址的主机进行单播回应。
 * Sender / Target Addresses：发送端和接收端的地址（ip+mac）
 * [parse_ARP](./03_rawsocket_underlying_c/02_Pointer_Casting&&Decapsulation/02_parse_ARP.c)中展示了如何去将arp的核心参数拆解出来。我们在这里处理协议解析的时候会基于底层的解析来进行，这一方面是为了展现该部分内容的连贯性和底层的封包直观，还是为了凸显c语言通过指针位移进行数据锚定的操作。
    ```c
      struct ether_arp *arp = (struct ether_arp*)(buf + sizeof(struct ethhdr));
    ```
  * 和上一节一样并且在之后将一直使用的是，我们将通过包装结构体来对特定的协议头进行解析，从本节最开头的展示可以看到，网络结构体在实际的物理地址中连续排列，因此我们只需要将数组的开头元素(这时候退化成指针)位移前一个网络头的大小，而这就体现出来了采用封装结构体的另一个好处，可以直接位移相应结构体的大小而不必直接位移具体的数据头大小(在这个示例中是14字节)，这一点大大降低了我们的出错概率提高了代码的可阅读性。
  * 在后续对arp头的解析乏善可陈，将结构体中的封装数据取出来进行打印展示
  //todo 进一步补充这里对arp数据包的解析示例
