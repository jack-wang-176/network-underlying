...
> **在第三部分（Part 3）中**，项目通过 C 语言的原始套接字（Raw Socket）深入协议栈底层。通过绕过操作系统的标准传输层，手动构造、发送并解析底层的 IP、TCP 与 UDP 数据包。这一部分为从零构建自定义的 TCP/IP 协议栈提供了实践基础，打通了从原始数据链路层帧到应用层网络流的理解闭环。
...
- [01 Preparation for a Deep Dive into the Protocol Stack(深入协议栈的准备)](#1深入协议栈的准备)
- [02 Pointer Casting and Decapsulation(指针强转与解封装)](#2指针强转与解封装)
- [03 send handshake package of tcp](#3-tcp-握手包的发送)
- [04 Summary](#4-总结)
...
到这里，我们成功实现了网络层（Network Layer）的核心探秘，视野也从单纯的局域网内物理 MAC 寻址，走向了广阔的全球互联网 IP 路由体系。在后续的章节中，我们将站在 IP 协议的肩膀上，深入探究具体的端到端通讯建立过程和数据包发送机制，去亲眼见证在第一部分我们习以为常的高级 API（如 `send`、`recvfrom`）在底层究竟是如何被组装和调度的。
