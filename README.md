...
### 3.TCP握手包的发送
* 在之前的数据包拆解部分，我们只需要去对一个完善的数据包进行指针偏移并套上对应的协议结构体包装，而在这部分我们则需要深入之前我们所忽略掉的技术细节，感受到计算机协议栈是如何规保证数据发送正确的。
* 首先让我们先去关注协议结构体，例如数据链路层的ethhdr,如果你尝试去手写这个结构体
  ```c
    struct my_ethhdr {
  ...
  ```
* 你会理所当然的认为这个结构体是14个字节，但是当你去尝试用printf进行打印的时候，却会显示是16个字节，这一点是因为在cpu内存总线寻址的时候更习惯与以4字节或者8字节进行吞吐，所以编译器会自动在后面加上两个字节，而这就是为什么<linux/ether.h>包或其他封装协议结构体的头文件中的结构体后面都会有__attribute__((packed))，添加上这个修饰符之后，我们才在真正意义上用c中的结构体严谨的映射出机器的连续内存。
* 第二点则是校验和。这一个字段主要用于检验数据包是否在传输时内容损失。在高级语言中我们理所当然的相信数据传输的可靠性，但是在真实的物理传输中电信号会衰减，射线会反转数据包内的比特位，路由器会因为拥塞和阻断数据。面对混杂的物理网络，内核不相信任何未经过数据自证的证据，而tcp/ip协议中通过16位反码求和的校验和来保障可靠的逻辑连接，在发送的时候预先填入校验和，在解析的时候有内核取出重新计算校验和并比对。
    ```c
    ...
    ```
* 在ip协议中校验和只需要去处理协议头部分，我们首先明白这段代码的主要目的：把数据当成一堆 16 位的数字，全部加起来，如果有进位就把进位加回最低位，最后按位取反。这样一个设计思路主要时满足互联网诞生之处cpu较低的执行速度，加法的执行速度较快，最后数据只需封装到一个2字节的数据结构中，并且无论时大段存储还是小端存储都能适用。
* 需要注意的是，数据校验并非只有网络层承担，数据链路层通过crc/fcs实现更严格的比特位级别的纠错，应用层的安全篡改则使用顶层tsl/ssl处理，ip/tcp层的网络校验的核心原则就是用最少的 CPU 时钟周期，换取足够用的错误拦截率。
* 我们来看一下这段看似简单实则极为巧妙的代码。在这里使用unsigned short 来对数据进行二字节的切分，使用unsigned int 类型来对加法结果进行储存，这里要将进位后的数据也储存起来。最后的结果依旧是16位的，因此只需要用unsigned short类型。
* 首先将数据切分并直接相加。其次将数据的前16位和后16位进行相加，(sum >> 16)取出进位数据，(sum & 0xFFFF)则将底层的数据清0，sum +=(sum >> 16)则处理上次相加过程中的再次溢出问题。
* 到这里为止我们已经成功的处理了ip协议中涉及的校验和，当我们去考虑到tcp字段的校验的时候，情况又有所不同。在处理tcp字段的校验和的时候同样的要求我们去计算ip字段，但是当你熟悉了协议拆解流程之后你就会发现，封装tcp头的时候ip头还没被加上去。
* 在开始对这一点进行解释之前我们先来解释一下tcp中为什么要这样设计，正如我们之前所说，在ip协议头中有一个ttl计算，当进行路由器跳转的时候ttl被自动减一，同时因为这个字段发生改变，所以检验和会被重新计算，但如果(在极少数情况下)路由器的硬件发生故障导致ip位改变，这个时候整个数据包还是成功校验通过。所以，在tcp中需要额外的ip段的封装，来避免动态计算校验和对结果的影响。
* 为了实现在没有封装tcp就使用ip头的越权问题，我们需要在代码内部定义出这个在实际网络线缆中并不存在，只在内存计算中产生作用的伪头部。
  ```c
    // 伪首部：IP 层的越权产物
