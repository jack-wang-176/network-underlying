...
* 至此，我们利用 Raw Socket 成功地逆向完成了整个协议栈的拼图，从零手搓并发送了一个合法的 TCP SYN 握手数据包。在这个过程中，我们击穿了高级网络库的封装幻象，亲历了处理结构体内存对齐的陷阱、手算了 IP 与 TCP 的双重校验和、并利用伪首部解决了跨层验证的难题。
掌握了这项直接“在网线上涂鸦”的底层能力后，你的视野绝不再局限于发送一个简单的握手包——只要你能遵守对应协议的内存排布法则，你可以随意修改代码中的控制标志位（Flags）与载荷（Payload），去自由地构造 RST 攻击包、伪造源 IP 数据报、甚至自己去实现一套专属于你的自定义网络协议。底层的广阔大门，此刻已向你完全敞开！
### 4.总结
现在让我们将视角先转会第一部分的内容(我们在这里先不考虑udp，因为相对于tcp而言udp的结构要简单很多)。[tcp_server](./01_socket_underlying_c/05_tcp/02_server.c),[tcp_client](./01_socket_underlying_c/05_tcp/01_client.c)，我们在一整章的内容只是去聚焦于单个数据包的构建和拆解，却忽略了
